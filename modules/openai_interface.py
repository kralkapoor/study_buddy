...
def create_revision_questions(summary):
    ...
    with open('revision_questions.txt','a') as file_output:
...
def transcribe():
    ...
    with open('full_trancript.txt','a') as file:
        for text_chunk in transcriptions:
            file.write(text_chunk)

    return transcriptions

def fetch_audio_chunks(path = 'audio_chunks'):
    chunked_audio_files = [file for file in os.listdir(path) if file.endswith('.mp3')]
